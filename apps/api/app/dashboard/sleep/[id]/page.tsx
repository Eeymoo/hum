...
export default function SleepDetailPage() {
  ...
  if (error === 'not_found') {
    return (
      <div className="p-6">
        <Link href="/dashboard/sleep" className="text-emerald-600 hover:text-emerald-800 mb-4 inline-block">← {t('title')}</Link>
  ...
  const stats = [
    { label: t('duration'), value: `${data.duration}h` },
    { label: t('quality'), value: `${data.quality}/10` },
    { label: t('bedTime'), value: data.bedTime },
    { label: t('wakeTime'), value: data.wakeTime },
    { label: t('deepSleep'), value: data.deepSleep != null ? `${data.deepSleep}h` : null },
    { label: t('remSleep'), value: data.remSleep != null ? `${data.remSleep}h` : null },
    { label: t('awakenings'), value: data.awakenings != null ? `${data.awakenings}` : null },
    { label: t('feeling'), value: data.feeling != null ? `${data.feeling}/10` : null },
  ]

  return (
    <div className="p-6">
      <Link href="/dashboard/sleep" className="text-emerald-600 hover:text-emerald-800 mb-4 inline-block">
        ← {t('title')}
      </Link>
        ...
        <Card className="mb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {data.attachments.map((att, i) => (
              <a key={i} href={`/api/v1/files/sleeps/${att.filename}`} target="_blank" rel="noopener noreferrer">
                <img
                  src={`/api/v1/files/sleeps/${att.filename}`}
